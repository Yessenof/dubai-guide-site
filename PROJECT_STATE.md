...
Last updated: 2026-08-07 (Phase 6D-AUG-NEW-02-SOURCE-LABEL-FIX-01 COMPLETE. Fixed a source-label/URL mismatch on AUG-NEW-02 found by a post-hotfix read-only consistency check: source_label_en/ru named "UAE Government Media Office" while source_url/cta_url pointed to u.ae (UAE Government Portal), not a Media Office page. Corrected source_label_en → "UAE Government Portal · Cabinet Resolution No. 27/2024" and source_label_ru → "Правительство ОАЭ · Постановление Кабинета № 27/2024" — 2 fields only. date (2026-08-28), confirmed status, source_url, cta_url, brief_en/ru all verified unchanged. Production access path corrected: db-backup-from-server.sh points at a decommissioned Cloudways host (165.245.187.15, decommissioned April 2026) — real production is UpCloud 85.9.203.69 (/var/www/guidex), reachable via existing SSH key auth. Patch script: scripts/patch-aug-new-02-source-label-fix.ts (idempotent, rehearsed against a fresh production DB copy first). Production backup: /var/www/guidex/backups/local/guides.db.pre-aug-new-02-source-label-fix-2026-08-07T13-15-03. Post-checkpoint DB MD5: 0d0bb71c7d553523b2b417ba6d185a10. Rebuild + pm2 reload performed (route has no revalidate export, matches prior hotfix precedent). EN+RU live QA PASS via direct curl (WebFetch tool's 15-min cache served stale content on first check — noted for future sessions). Implementation commit: e6ef153. Hotfix report: docs/content-drafts/seo/6d-aug-new-02-source-label-fix-01.md. KNOWN ISSUE: local data/guides.db still stale (predates both August hotfixes, date=2026-08-25) — git-ignored, does not participate in deploy, no immediate hazard, but db-backup-from-server.sh needs its server target updated before it can refresh it. NEXT: Architecture Revision 02 (PLAN ONLY, owner review of freshness architecture plan first) — do not begin Phase 6E. Previously: Phase 6D-MAWLID-OFFICIAL-CONFIRMATION-HOTFIX-01 COMPLETE — AUG-NEW-02 date corrected 2026-08-25 → 2026-08-28 (confidence: expected → confirmed). UAE Government Media Office announced 28 Aug 2026 as official Mawlid Al Nabi public holiday for federal + private sector. Patch script: scripts/patch-aug-mawlid-official-confirmation-2026.ts. DB MD5 post-WAL-checkpoint: c09f358ff2b75ac7f87ecf87ca9805a9. Implementation commit: 7265b76. Hotfix report: docs/content-drafts/seo/6d-mawlid-official-confirmation-hotfix-01.md. Previously: Phase 6D-PRODUCTION-DEPLOY-CLOSURE-01 COMPLETE. Documentation corrections: Sep new count +4 not +8 (pre-6D Sep was 12 not 8); hreflang on calendar monthly pages CORRECT (hrefLang camelCase, 3 tags, reciprocal); dedicated F1 event page /events/formula-1-abu-dhabi-grand-prix-2026 EXISTS and is published (previous report wrong); GITEX slug gitex-global-2026 (tested wrong slug in previous QA); memory-guard.sh permissions correct on server (local Mac TCC restriction). Hub hreflang partial-implementation gap added to P1 backlog. git bundle 48bc3c3 created for local transfer. Phase 6D fully CLOSED. NEXT: scp bundle to local Mac, fast-forward main, push to GitHub; then GSC Wave 1 manual submission (24h stability window from 2026-08-07 10:07 UTC). Previously: Phase 6D-CALENDAR-Q3Q4-2026-PRODUCTION-DEPLOY-01 COMPLETE.

Previously: 2026-07-19 (Phase 6C-GSC-INDEXING-RECOVERY-03-PROD COMPLETE — Deployed f11ec5b to production. All 6 hub pages verified live: no noindex, self-canonical, bidirectional hreflang, all in sitemap. Sitemap lastmod uses SITE_BUILD constant (2026-07-19). Full site HTTP audit: 108/108 URLs return 200. Prod report: docs/content-drafts/seo/6c-gsc-indexing-recovery-03-prod.md.)

Previously: 2026-07-19 (Phase 6C-GSC-INDEXING-RECOVERY-03 COMPLETE — 7 code files changed; 0 TS errors; local only, not yet deployed. Fixes: (1) Removed noindex from 6 hub pages — /events, /ru/events, /news, /ru/news, /calendar, /ru/calendar — all had Phase 3B legacy noindex but now contain real content; (2) Added EN↔RU hreflang to events and news hubs (calendar hubs already had it); (3) Added 6 hub pages to sitemap.ts; (4) Fixed sitemap lastModified: new Date() → static SITE_BUILD = new Date("2026-07-19"). Audit doc: docs/content-drafts/seo/6c-gsc-indexing-recovery-03.md. Next: commit + push, then await separate production deployment approval.)

Previously: 2026-07-18 (Phase 6C-INFRA-UPCLOUD-DNS-CUTOVER-01 COMPLETE — Confirmed guidex-consulting.ae → 85.9.203.69 already live (DNS set April 2026, no change needed). All Batch 01ABC content live: Etihad Rail guide (5 steps EN/RU hreflang), July calendar 10 items (JUL-NEW-04/05/06/07 added), August Mawlid AUG-NEW-02 (date=2026-08-25, u.ae source). SSL valid to 2026-09-26. PM2 online, 16/16 QA routes 200. guidex.ae secondary domain still → 139.162.173.118 (not canonical, left unchanged). Production HEAD: ef8b58f. Recommended next: guidex.ae cleanup at Tasjeel + Phase 6C-GSC-INDEXING-RECOVERY for new URLs.)
