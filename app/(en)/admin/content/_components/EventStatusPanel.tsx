...
} from "@/app/(en)/admin/content/actions/events";

const INITIAL: EventActionState = { errors: [], warnings: [] };
...
export default function EventStatusPanel({
  ...
  const [publishState, publishAction] = useActionState(publishEventAction, INITIAL);
  const [archiveState, archiveAction] = useActionState(archiveEventAction, INITIAL);
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <span className="text-xs text-gray-400">Date confidence</span>
        <DateConfidenceBadge confidence={dateConfidence} />
      </div>

      {dateConfidence !== "confirmed" && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
          <p className="text-xs text-amber-700">
            Non-confirmed date: ensure en_body includes a plain-language disclaimer about date uncertainty.
          </p>
        </div>
      )}
