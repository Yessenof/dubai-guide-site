/**
 * Phase 6C-CALENDAR-EXPANSION-04B -- Approved P0 Import (Production)
 * PRODUCTION script -- run on server via: npx tsx scripts/import-calendar-expansion-04b-production.ts
 *
 * Writes:
...
section("Phase 6C-CALENDAR-EXPANSION-04B -- Production Import");
...
const BACKUP_PATH = `${DB_PATH}.backup-pre-04b-prod-${TS}`;
...
if (decEnsAlready) {
  ...
  log(`  DEC-ENS appended + em-dash cleanup + published. Warnings: ${decPub.warnings.length ? decPub.warnings.join("; ") : "none"}`);
}
...
if (!ensRow || ensRow.status !== "published") {
  ...
  log(`  ${ENS_SLUG}: status=${ensRow.status}  dates=${ensRow.eventDateStart}/${ensRow.eventDateEnd}  PASS`);
}
...
if (verDecPage) {
  ...
  if (!decEns) { console.error("  FAIL: DEC-ENS not in December calendar."); anyFail = true; }
  else { log(`  DEC-ENS in December: date=${decEns["date"]}  PASS`); }
}
...
if (verNovPage) {
  ...
  if (!novGfmfg) { console.error("  FAIL: NOV-GFMFG not in November calendar."); anyFail = true; }
  else { log(`  NOV-GFMFG in November: date=${novGfmfg["date"]}  PASS`); }
}
...
if (verAugPage) {
  ...
  if (augHasIsl !== 1 && augHasIsl !== true) {
    console.error(`  FAIL: has_islamic_dates still ${augHasIsl}.`); anyFail = true;
  } else {
  ...
  if (!verAugNew02 || verAugNew02["source_status"] !== "expected") {
...
section("Production import complete -- ALL PASS");
log(`  DB: ${DB_PATH}`);
log(`  Backup: ${BACKUP_PATH}`);
log(`  Event: ${ENS_SLUG}  id=${ensId}`);
