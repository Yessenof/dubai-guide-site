 ...
 *   Then restore calendar dates_json from backup: cp data/guides.db.backup-pre-6c97e-* data/guides.db
 */
...
section("Phase 6C-97E -- High-Value Event Pages Local Import QA");
log(`  DB path:   ${DB_PATH}`);
...
const BACKUP_PATH = `${DB_PATH}.backup-pre-6c97e-${TS}`;
...
const ALL_STRINGS: Array<[string, string]> = [
  ["GITEX_EN_SEO_TITLE", GITEX_EN_SEO_TITLE],
  ["GITEX_EN_META",      GITEX_EN_META],
  ["GITEX_EN_TITLE",     GITEX_EN_TITLE],
  ["GITEX_EN_SUMMARY",   GITEX_EN_SUMMARY],
  ["GITEX_EN_BODY",      GITEX_EN_BODY],
  ["GITEX_RU_SEO",       GITEX_RU_SEO],
  ["GITEX_RU_METAD",     GITEX_RU_METAD],
  ["GITEX_RU_TITLE",     GITEX_RU_TITLE],
  ["GITEX_RU_SUMM",      GITEX_RU_SUMM],
  ["GITEX_RU_BODY",      GITEX_RU_BODY],
  ["F1_EN_SEO_TITLE",    F1_EN_SEO_TITLE],
  ["F1_EN_META",         F1_EN_META],
  ["F1_EN_TITLE",        F1_EN_TITLE],
  ["F1_EN_SUMMARY",      F1_EN_SUMMARY],
  ["F1_EN_BODY",         F1_EN_BODY],
  ["F1_RU_SEO",          F1_RU_SEO],
  ["F1_RU_METAD",        F1_RU_METAD],
  ["F1_RU_TITLE",        F1_RU_TITLE],
  ["F1_RU_SUMM",         F1_RU_SUMM],
  ["F1_RU_BODY",         F1_RU_BODY],
];
...
for (const slug of [GITEX_SLUG, F1_SLUG]) {
  const found = existingEvents.find(e => e.slug === slug);
  if (found) {
    log(`  SKIP: Event slug "${slug}" already exists (id=${found.id}, status=${found.status}).`);
    log("  This script is idempotent: existing slugs are skipped, not duplicated.");
  } else {
    log(`  OK: "${slug}" not found -- safe to create.`);
  }
}

const existingCals = getAllCalendarPages();
const decPage = existingCals.find(p => p.slug === DEC_CAL_SLUG);
if (!decPage) abort(`Calendar page "${DEC_CAL_SLUG}" not found in local DB.`);
...
let gitexId: string | null = null;
const existingGitex = existingEvents.find(e => e.slug === GITEX_SLUG);

if (existingGitex) {
  gitexId = existingGitex.id;
  log(`  SKIP: Already exists. id=${gitexId}  status=${existingGitex.status}`);
} else {
...
if (existingF1) {
  f1Id = existingF1.id;
  log(`  SKIP: Already exists. id=${f1Id}  status=${existingF1.status}`);
} else {
...
const DETAIL_URL_UPDATES: Record<string, string> = {
...
// Re-fetch calendar page for fresh JSON
const freshCals = getAllCalendarPages();
const freshDecPage = freshCals.find(p => p.slug === DEC_CAL_SLUG);
if (!freshDecPage) abort("december-2026-uae-calendar disappeared after event inserts.");

const items = JSON.parse(freshDecPage.datesJson) as Array<Record<string, unknown>>;
let updatedCount = 0;
const updatedIds: string[] = [];

for (const [targetId, targetUrl] of Object.entries(DETAIL_URL_UPDATES)) {
  const idx = items.findIndex(x => x["id"] === targetId);
  if (idx === -1) {
    log(`  WARN: Item "${targetId}" not found in dates_json. Skipping.`);
    continue;
  }
...
if (updatedCount > 0) {
  ...
  log("  No calendar updates needed (all detail_urls already set).");
}
...
// Verify detail_urls in calendar
...
if (verDecPage) {
  ...
  for (const [tid, turl] of Object.entries(DETAIL_URL_UPDATES)) {
    const found = verItems.find(x => x["id"] === tid);
    if (!found) {
      console.error(`  FAIL: ${tid} missing from calendar.`);
      anyFail = true;
...
EVENTS INSERTED AND PUBLISHED (2):
  ...
  GITEX:  id=${gitexId}
          ...
          related_guide=mainland-company-setup-dubai
  ...
  F1:     id=${f1Id}
          ...
          category=event  color_type=major-event

CALENDAR ITEMS UPDATED (detail_url set):
...
ROLLBACK INSTRUCTIONS:
  sqlite3 data/guides.db \\
    "DELETE FROM events WHERE slug IN ('${GITEX_SLUG}','${F1_SLUG}');"
