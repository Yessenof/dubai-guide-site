/**
 * Phase 6C-97B -- UAE Calendar Batch 2B Local Import QA
 * LOCAL DEVELOPMENT ONLY -- do NOT run against production.
 *
 * September (+1):
 *   SEP-R1   The Corrs, Etihad Arena Abu Dhabi, Sep 27
 *
 * October (+2):
 *   OCT-R1   Elrow Dubai, Dubai Media City Amphitheatre, Oct 24
 *   OCT-R2   Boris Grebenshikov, The Agenda Dubai Media City, Oct 24
 *
 * November (+8):
 *   NOV-R1   Dubai Ride (DFC opener), Nov 1
 ...
 *
 * December (+1 new, +1 update):
 *   DEC-R1          Imagine Dragons, Etihad Park Yas Island, Dec 5 (F1 Yasalam)
 *   DEC-UPDATE-1    Update existing DEC-NEW-01 (Lewis Capaldi) to add Zara Larsson
 *
 ...
 *
 * Run: npx tsx scripts/import-uae-calendar-batch-2b-local-6c97b.ts
 */
...
const BACKUP_PATH = `${DB_PATH}.backup-pre-6c97b-${new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19)}`;
...
section("Phase 6C-97B -- UAE Calendar Batch 2B Local Import");
log(`  DB path:     ${DB_PATH}`);
...
log(`  Backup size: ${Math.round(fs.statSync(BACKUP_PATH).size / 1024)}K`);

// ---- Guards -----------------------------------------------------------------

const EM = "—"; // em dash — forbidden in calendar labels
...
const HARD_EXCLUSIONS = new Set([
  "GLOBAL-VILLAGE", "DSF", "TIMUR-BEY", "BEAT-THE-HEAT",
  "CCA-DEC16", "KADIM", "SHM", "ATB-SEP18",
]);
...
// ---- Pre-flight: guards on new items ----------------------------------------

section("Pre-flight validation -- new items");

for (const item of BATCH_2B_NEW) {
  if (HARD_EXCLUSIONS.has(item.id)) abort(`Hard-excluded item "${item.id}" found.`);
  for (const [k, v] of Object.entries(item)) {
    if (typeof v === "string") assertNoEmDash(`${item.id}.${k}`, v);
  }
}
log(`  ${BATCH_2B_NEW.length} new items validated. No em dashes. No exclusions. PASS`);
...
for (const [slug, newItems] of Object.entries(INSERTS_BY_SLUG)) {
  ...
  if (!page) abort(`Page not found: "${slug}"`);
...
// ---- Step 2: Update DEC-NEW-01 (add Zara Larsson) ---------------------------

section("Step 2 -- Update DEC-NEW-01 (add Zara Larsson to Dec 3 F1 concert)");

const decPage = getAllCalendarPages().find(p => p.slug === "december-2026-uae-calendar");
...
// Validate no em dashes in patch values
...
if (!decUpd.ok) abort(`updateCalendarDraft failed for December: ${JSON.stringify(decUpd.errors)}`);

const decPub = publishCalendar(decPage.id);
if (!decPub.ok) abort(`publishCalendar failed for December: ${JSON.stringify(decPub.errors)}`);
...
for (const slug of slugsToCheck) {
  ...
  if (!p || p.status !== "published") abort(`${slug} not published after import.`);
  const items = JSON.parse(p.datesJson) as DateItem[];
  log(`  ${slug}: status=${p.status}, items=${items.length}`);
  // Print all item IDs for transparency
  for (const it of items) {
    log(`    [${it.id}] ${it.date} -- ${it.short_label_en}`);
  }
}

// Verify DEC-NEW-01 update
...
if (decPageAfter) {
  ...
  if (!decNew01) abort("DEC-NEW-01 missing after update.");
  if (!decNew01.label_en.includes("Zara Larsson")) abort("DEC-NEW-01 update: Zara Larsson not in label_en.");
  if (!decNew01.label_ru.includes("Zara Larsson")) abort("DEC-NEW-01 update: Zara Larsson not in label_ru.");
  log(`\n  DEC-NEW-01 label_en: ${decNew01.label_en}`);
  log(`  DEC-NEW-01 label_ru: ${decNew01.label_ru}`);
  log(`  DEC-NEW-01 short_en: ${decNew01.short_label_en}`);
  log(`  Zara Larsson present in DEC-NEW-01: PASS`);
}
...
By month:
  ...
  october-2026-dubai-calendar    +2 (OCT-R1 -- Elrow, OCT-R2 -- Boris Grebenshikov)
...
Expected item counts after import:
  ...
  October:    13  (was 11)
  November:   14  (was 6)
  December:   7   (was 6, plus DEC-NEW-01 updated)
...
ROLLBACK:
  ...
  -- OR: use scripts/rollback.sh on production
