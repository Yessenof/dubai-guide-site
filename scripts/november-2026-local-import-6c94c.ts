/**
 * Phase 6C-94C -- November 2026 Local Import QA
 * LOCAL DEVELOPMENT ONLY -- do NOT run against production.
 *
 * Imports (local DB only):
 ...
 *
 * Run: npx tsx scripts/november-2026-local-import-6c94c.ts
 */
...
// ---- Calendar dates_json (3 items -- Downtown Design HOLD, DFC HOLD) ----

const DATES_JSON = JSON.stringify([

  // NOV-04-ADIPEC -- ADIPEC 2026, Abu Dhabi (Nov 2-5 -- before DDW starts)
  {
    id: "NOV-04-ADIPEC",
    date: "2026-11-02",
...
const ALL_STRINGS: Array<[string, string]> = [
  ["DDW_EN_TITLE",      DDW_EN_TITLE],
  ["DDW_EN_SUMMARY",    DDW_EN_SUMMARY],
  ["DDW_EN_BODY",       DDW_EN_BODY],
  ["DDW_EN_SEO_TITLE",  DDW_EN_SEO_TITLE],
  ["DDW_EN_META",       DDW_EN_META],
  ["DDW_RU_TITLE",      DDW_RU_TITLE],
  ["DDW_RU_SUMMARY",    DDW_RU_SUMMARY],
  ["DDW_RU_BODY",       DDW_RU_BODY],
  ["DDW_RU_SEO_TITLE",  DDW_RU_SEO_TITLE],
  ["DDW_RU_META",       DDW_RU_META],
  ["B5_EN_TITLE",       B5_EN_TITLE],
  ["B5_EN_SUMMARY",     B5_EN_SUMMARY],
  ["B5_EN_BODY",        B5_EN_BODY],
  ["B5_EN_SEO_TITLE",   B5_EN_SEO_TITLE],
  ["B5_EN_META",        B5_EN_META],
  ["B5_RU_TITLE",       B5_RU_TITLE],
  ["B5_RU_SUMMARY",     B5_RU_SUMMARY],
  ["B5_RU_BODY",        B5_RU_BODY],
  ["B5_RU_SEO_TITLE",   B5_RU_SEO_TITLE],
  ["B5_RU_META",        B5_RU_META],
...
// ---- Pre-flight: slugs do not exist ----------------------------------------

section("Pre-flight: confirm slugs do not already exist");
...
for (const slug of [DDW_EVENT_SLUG, B5_EVENT_SLUG]) {
  ...
  if (found) {
    ...
    console.error("       Delete the existing row or run rollback before re-importing.");
    process.exit(1);
  }
}
...
section("Validate dates_json item IDs");
...
const dfc = parsedDates.find(d => d.id?.includes("DFC") || d.label_en?.toLowerCase().includes("fitness challenge"));
if (dfc) {
  console.error(`\nABORT: DFC found in DATES_JSON. DFC is HOLD (site 403) -- do not import.`);
  process.exit(1);
}
const dd = parsedDates.find(d => d.id === "NOV-02-DD" || d.label_en?.toLowerCase().includes("downtown design") && d.id !== "NOV-01-DDW");
if (dd) {
  console.error(`\nABORT: Downtown Design found as separate item. Only mention within DDW label -- not as standalone.`);
  process.exit(1);
}
log(`  ${parsedDates.length} items, no duplicates. IDs: ${[...seenIds].join(", ")}.`);
log(`  DFC check: not present. ✓`);
log(`  Downtown Design check: not as standalone item. ✓`);
...
EVENTS CREATED AND PUBLISHED (2):
  ...
  DDW:  id=${ddwId}
        ...
        ru_url=/ru/events/dubai-design-week-2026
  ...
  Big5: id=${b5Id}
        ...
        ru_url=/ru/events/big-5-global-dubai-2026
...
CALENDAR CREATED AND PUBLISHED (1):
  ...
  Calendar: id=${calId}
            ...
            ru_url=/ru/calendar/november-2026-dubai-calendar
            month=11 year=2026 type=monthly
            last_verified_date=2026-06-01

DATES_JSON ITEMS (3 total):
  NOV-04-ADIPEC  ADIPEC 2026 (Abu Dhabi)         2026-11-02  conference   L2 brief
  NOV-01-DDW     Dubai Design Week 2026           2026-11-03  trade_show   L2 brief
  NOV-03-BIG5    Big 5 Global 2026                2026-11-23  trade_show   L2 brief
...
COVERAGE (3 items, DFC and Downtown Design held):
  Nov 2-5 (ADIPEC, Abu Dhabi) + Nov 3-8 (DDW) + Nov 23-26 (Big5) = 11 unique days
  11/30 = 36.7% (below target; documented. DFC resolution would bring to ~93%)

ROLLBACK (local only):
  To remove these local rows:
  sqlite3 data/guides.db "DELETE FROM events WHERE slug IN ('dubai-design-week-2026','big-5-global-dubai-2026');"
  sqlite3 data/guides.db "DELETE FROM calendar_pages WHERE slug='november-2026-dubai-calendar';"

LOCAL ONLY. No push. No deploy. No production DB write.
Production import requires separate owner approval (Phase 6C-94D).
`);
