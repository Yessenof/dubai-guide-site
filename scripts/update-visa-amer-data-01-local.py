#!/usr/bin/env python3
"""
Phase 6C-VISAS-AMER-DATA-INTEGRATION-01 -- Local DB update
Integrates AMER practical data into employment-visa and golden-visa-dubai-property.
LOCAL ONLY -- do not run on production until the 6C-VISAS-AMER-DATA-INTEGRATION-01-PROD phase.

Usage: python3 scripts/update-visa-amer-data-01-local.py
"""
...
BACKUP  = DB_PATH.parent / f'guides.db.backup-pre-amer-data-integration-01-{TS}'
...
if 'var/www' in str(DB_PATH):
    abort('This is the LOCAL script. Use the production script for the server.')

sep('Phase 6C-VISAS-AMER-DATA-INTEGRATION-01 -- Local DB Update')
print(f'  DB:  {DB_PATH}')
print(f'  TS:  {TS}')
...
STEP_UPDATES = {
    ...
    # ── employment-visa Step 3: Entry permit inside ───────────────────────────
    ...
    # ── employment-visa Step 5: Medical ──────────────────────────────────────
    ...
    # ── employment-visa Step 8: Stamping ─────────────────────────────────────
    ...
    # ── golden-visa Step 2: Prepare docs ─────────────────────────────────────
    ...
    # ── golden-visa Step 4: Submit application ───────────────────────────────
    ...
    # ── golden-visa Step 7: Family sponsorship ───────────────────────────────
...
for guide_id, slug, en_new, ru_new in [
    ...
    for f in FORBIDDEN:
        for field, text in [('en_overview', en_ov_check), ('ru_overview', ru_ov_check)]:
            if f in text:
                print(f'  FAIL: {slug} {field} contains forbidden "{f}"', file=sys.stderr)
                any_fail = True
    if not any_fail:
...
if any_fail:
    abort('One or more verification checks failed.')
...
print()
print('  Next: run build and local QA')
print('    npm run build')
