#!/usr/bin/env python3
"""
Phase 6C-VISAS-CONTENT-OPT-01 -- Visa Content Practical Additions (Production)
...
Run on server:
  cd /var/www/guidex
  python3 scripts/update-visa-content-opt-01-production.py
"""
...
BACKUP  = DB_PATH.parent / f'guides.db.backup-pre-visa-opt-01-prod-{TS}'
...
sep('Phase 6C-VISAS-CONTENT-OPT-01 -- Production DB Update')
print(f'  DB:  {DB_PATH}')
print(f'  TS:  {TS}')
...
# ── Verify step IDs exist on this server's DB ──────────────────────────────────

sep('Verifying step IDs exist in production DB')
for step_id in STEP_UPDATES:
    cur.execute('SELECT id FROM steps WHERE id = ?', (step_id,))
    if not cur.fetchone():
        abort(f'Step ID not found in production DB: {step_id} -- DB may be out of sync. Abort safe.')
...
for slug, data in GUIDE_UPDATES.items():
    ...
    if cur.rowcount != 1:
        abort(f'Expected 1 row for {slug}, got {cur.rowcount}')
...
FORBIDDEN = ['  --', 'AED 10,500', 'AED 5,000 deposit', '48% share',
             'Ukraine 1-year', 'official fee AED 15,000', 'guaranteed']
...
for slug, data in GUIDE_UPDATES.items():
    ...
    for text in [en_ov, ru_ov]:
        for f in FORBIDDEN:
            if f in text:
                print(f'  FAIL: {slug} contains forbidden: "{f}"', file=sys.stderr); any_fail = True
    if not any_fail:
        print(f'  {slug}: PASS')
